...
class Printer():
	...
	def replace_character(self, input_string, old_char, new_char):
		return re.sub(old_char, new_char, input_string)

	def print_card(self, data, print_type="sideways"):
		if data is None:
		...
		type_line = self.replace_character(data["type_line"], "—", "-")
		oracle_text = data["oracle_text"]
		oracle_text = self.replace_character(oracle_text, "—", "-")
		oracle_text = self.replace_character(oracle_text, "•", "-")
		oracle_text = self.replace_character(oracle_text, "\n", "\n\n")
	...
	def print_random(self):
		return self.print_card(get_random_card())

	def print_random_by_query(self, query):
		return self.print_card(get_random_card_by_query(query))

	def print_random_creature_by_cmc(self, cmc):
		query = f"t:creature cmc={cmc}"
		card = get_random_card_by_query(query)
		return self.print_card(card)

	def print_card_by_name(self, name):
		return self.print_card(get_card_by_name(name))

	def print_image(self, image):
		return print_request(image, print_type="image")
